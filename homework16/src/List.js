let posts = [
  {
    name: "Pumpkin",
    icon: "https://cdn-icons-png.flaticon.com/512/616/616430.png",
    nickname: "@king.kat",
    content: "am relaxing...",
    image:
      "https://images.unsplash.com/photo-1552954877-e96a0f0728c0?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxzZWFyY2h8NHx8dGFiYnklMjBjYXR8ZW58MHx8MHx8&w=1000&q=80",
    date: "January 20",
  },
  {
    name: "Elvis",
    icon: "https://icones.pro/wp-content/uploads/2021/08/icone-de-chat-endormi.png",
    nickname: "@elvis.ilgatto",
    content: "My scratcher sofa is so comfortable",
    image:
      "https://media.wired.com/photos/6242576ba288ab4bb7de35ed/1:1/w_1825,h_1825,c_limit/PetFusion-Cat-Scratcher-Lounge-Gear.jpg",
    date: "February 3",
  },
  {
    name: "Blue",
    icon: "https://icons.veryicon.com/png/o/miscellaneous/taoist-music/cat-56.png",
    nickname: "@chooliet",
    content: "Lil cloud",
    image:
      "https://images.pexels.com/photos/45201/kitty-cat-kitten-pet-45201.jpeg?cs=srgb&dl=pexels-pixabay-45201.jpg&fm=jpg",
    date: "February 9",
  },
  {
    name: "Nugget",
    icon: "https://cdn-icons-png.flaticon.com/512/374/374980.png",
    nickname: "@orange_13",
    content: "King of the castle",
    image:
      "https://i.etsystatic.com/5376657/r/il/20dcc0/2376505716/il_570xN.2376505716_jkiu.jpg",
    date: "January 15",
  },
  {
    name: "Moon",
    icon: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRplDD4FBMJn89EAIaPzqOcBnnFQ-CRxdDoag&usqp=CAU",
    nickname: "@worldcatsblack",
    content: "My beautiful family 🖤🖤",
    image:
      "https://assets3.thrillist.com/v1/image/2911434/792x623/scale;webp=auto;jpeg_quality=60.jpg",
    date: "January 30",
  },
  {
    name: "Snow",
    icon: "https://cdn2.vectorstock.com/i/1000x1000/64/26/cat-simple-icon-black-and-white-vector-24986426.jpg",
    nickname: "@themonocats",
    content: "Friends",
    image:
      "https://i.pinimg.com/originals/cb/57/83/cb57837c5c1be346f33b40e2ffd53e9d.jpg",
    date: "February 6",
  },
];
export { posts };
